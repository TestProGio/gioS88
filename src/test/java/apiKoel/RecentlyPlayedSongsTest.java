...
import apiKoel.responses.RecentlyPlayedSongsResponse;
...
public class RecentlyPlayedSongsTest {
    ...
    public void testGetRecentlyPlayedSongs() {
        ...
        int count = 1; // Set the number of songs to be returned, if required.
        ...
        // Log the JSON response (equivalent to console.log(json))
        // Debugging output for validation
        System.out.println("Response Status Code: " + response.statusCode());
        System.out.println("Response Body: " + response.asString());

        // Deserialize response to POJO
        RecentlyPlayedSongsResponse recentlyPlayedSongs = response.as(RecentlyPlayedSongsResponse.class);

        // Soft assertions
        softAssert.assertNotNull(recentlyPlayedSongs.getSongIds(), "Response song list should not be null");
        if (recentlyPlayedSongs.getSongIds().isEmpty()) {
            softAssert.assertTrue(recentlyPlayedSongs.getSongIds().isEmpty(), "Expected empty list when no songs are played recently");
        } else {
            softAssert.assertTrue(recentlyPlayedSongs.getSongIds().size() <= count,
                    "The number of songs should not exceed the requested count");
            softAssert.assertTrue(recentlyPlayedSongs.getSongIds().stream().allMatch(id -> id.matches("[a-f0-9]{32}")),
